		...
		"captionify": function(element,defaultLanguage,options) {
			...
			if (!objectsCreated) {
				...
				captionator.TextTrack = function TextTrack(id,kind,label,language,trackSource,isDefault,type) {
					...
					this.type = type || "";    // Adding type so we will check if we need to get JSONP later on
					...
					this.loadTrack = function(source, callback) {
						...
						if (this.readyState === captionator.TextTrack.LOADED) {
							...
							// Check that currentTrack type is not a  jsonp
							if (this.type.match(/^jsonp\//)) {
				                captionator.getJSONP("".concat(source, "&callback=?"), function(data){
				                    if(data && data.length > 0) {
                                        data = captionator.JSONtoVTT(data);
                                        captionator.setCaptionsForCurrentTrack(data, currentTrackElement, callback);
                                    }
				                });
							} else {
				...
				captionator.setCaptionsForCurrentTrack = function setCaptionForCurrentTrack(data, currentTrackElement, callback) {
                ...
                captionator.JSONtoVTT = function(data) {
                    var vttData = "WEBVTT";
                    var vttChunk = "";
				...
				captionator.getJSONP = function getJSONP(URL, success) {
				...
				captionator.secondsToHmsm = function secondsToHms(totalSec) {
                    //totalSec = totalSec.toFixed(3);
                    
                    var hours = parseInt( totalSec / 3600 ) % 24;
                    var minutes = parseInt( totalSec / 60 ) % 60;
                    var seconds = (totalSec % 60).toFixed(3);
			...
			[].slice.call(document.getElementsByTagName("video"),0).forEach(function(videoElement) {
				videoElement.addTextTrack = function(id,kind,label,language,src,type,isDefault) {
					...
					type = typeof(type) === "string" ? type : "";  // Adding type here since we will use it later on
...
/* 				"position": "absolute", */
				...
				// Taking this line out to make Captionator responsive to UIO options
			...
			if (cueObject.direction === "vertical" || cueObject.direction === "vertical-lr") {
				...
				// Hacked those if statements
				// Before it was DOMNode.offsetHeight * 1.2
				// Removed this coefficient since it would not show an additional line in case when text size or line spacing is increased through UIO
				if (DOMNode.scrollHeight > DOMNode.offsetHeight) {
		...
		*/
		"styleCueCanvas": function(videoElement) {
			...
			// Style node!
			// Taking off the styling to make Captionator to be responsible for UIO options
			/*
