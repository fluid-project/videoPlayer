		...
		*/
		"captionify": function(element,defaultLanguage,options) {
			...
			if (!objectsCreated) {
					...
					this.loadTrack = function(source, callback) {
						...
						if (this.readyState === captionator.TextTrack.LOADED) {
							...
							if (this.type.match(/^jsonp\//)) {
				                captionator.getJSONP("".concat(source, "&callback=?"), "captionator.JSONPcallback");
							} else {
				...
				captionator.JSONPcallback = function(data) {
				...
				captionator.getJSONP = function getJSONP(URL, callbackName) {
                    document.getElementsByTagName('body')[0].appendChild((function(){
                        var s = document.createElement('script');
                        s.type = 'text/javascript';
                        s.src = URL.replace('callback=?','callback=' + callbackName);
                        return s;
                    })());
				};
